...
Furthermore, with the improved corpus creation method presented in Section~\ref{sec:covgran-ux22}, we achieve a new state-of-the-art reference matching success rate of 44.4\%. Different from our contributions to \rtmark{2} in Chapter~\ref{chp:corpus}, we are here able to make direct comparisons to related work. Our reference matching success rate of 44.4\% compares favorably to our previously method (42.6\%), S2ORC (31.1\%), and arXMLiv (no citation network). Accordingly, we 
Accordingly, we deem \rtmark{2} successfully achieved.
...
\begin{infobox-progress}
      ...
      \begin{tabular}{lp{10.9cm}}
        ...
        Crit. & Explanation \\
        \midrule
        $\mathbf{Rel_{SDR}}$ & Improved relevance, by adding mathematical notation as well as figure and table captions to the document representation \\
        $\mathbf{Tim_{C/S}}$ & Improved coverage, by including publications up until the end of the most recent full year \\
        $\mathbf{Coy_{CN}}$ & Improved comparability, by providing OpenAlex and arXiv IDs, as well as DOIs through the linked OpenAlex records \\
        $\mathbf{Coy_{SDR}}$ & Improved document representation comparability, by providing section structure and types \\
        $\mathbf{Cos_{CN}}$ & Improved citation network completeness, by introducing blocking and matching procedure \\
        $\mathbf{Cos_{SDR}}$ & Improved document representation completeness, by providing full-text with mathematical notation preserved as \LaTeX \\
        \bottomrule
      \end{tabular}
