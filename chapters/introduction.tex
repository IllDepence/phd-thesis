...
\begin{dqlist}
    \item $\mathbf{Q_{rel}}$: \textit{Relevance} quantifies how well the data is connected to its intended use.
    \item $\mathbf{Q_{acc}}$: \textit{Accuracy} is concerned with how error-free the data is.
    \item $\mathbf{Q_{tim}}$: \textit{Timeliness} expresses to which degree the data is current enough.
    \item $\mathbf{Q_{coy}}$: \textit{Comparability} measures the extent to which the data is comparable to other \hphantom{$\mathbf{Q_{coy}}$: }data, e.g. due to the existence of common identifiers.
    \item $\mathbf{Q_{cos}}$: \textit{Completeness} is a measure for the rate of missing records, and the rate of \hphantom{$\mathbf{Q_{cos}}$: }missing data elements within records.
\end{dqlist}
...
This dissertation is not the first research endeavor with the objective the develop methods for the creation of high-quality scholarly data. Prior approaches exist and, accordingly, have grappled with the challenges laid out in the previous section. However, there are three key areas in which we see particular shortcomings in existing work. Together, they make up the research gap addressed by this dissertation, and are described in the following.
% More detailed accounts regarding each area can be found in the respective chapters later on.
Based on the identified shortcomings, we then formulate our research tasks.
...
Based on the research gap identified above, we formulate the following four research tasks as sub-points to our overarching research objective (see Section~\ref{sec:intro-researchobj}).

\begin{rtlist}
    \item Base Methodology - identify or establish a base methodology for generating a large-scale high quality scholarly data set, on par with or exceeding existing data sets.
    \item Citation Network Completeness - develop a method to link literature references
    \item Inclusion of Non-English Publications - 
    \item Fine-gained Research Artifact Representations - 
\end{rtlist}
