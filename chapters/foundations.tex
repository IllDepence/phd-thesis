  ...
  \begin{small}
    \begin{threeparttable}
      \begin{tabular}{lll}  % p{9.7cm}
        \toprule
        General Dimension & Focus\tnote{a} & Specific Criterion\\
        \midrule
        \multirow{2}{*}{Relevance} & CN & Representative coverage of publications in target area \\
        \ & SDR & Inclusion of relevant content types (text, formulae, tables, etc.) \\
        \arrayrulecolor{lightgrey}\hline\arrayrulecolor{black}
        \multirow{2}{*}{Accuracy} & CN & Correctly linked references \\
        \ & SDR & Noise-free full-text content \\
        \arrayrulecolor{lightgrey}\hline\arrayrulecolor{black}
        \multirow{2}{*}{Timeliness} & \multirow{2}{*}{(both)} & \multirow{2}{*}{Coverage of recent publications} \\
        \ & \ & \ \\
        \arrayrulecolor{lightgrey}\hline\arrayrulecolor{black}
        \multirow{2}{*}{Comparability} & CN & Use of established document identifiers (DOI, PubMed ID, etc.) \\
        \ & SDR & Fine-granular, specifically typed content representation \\
        \arrayrulecolor{lightgrey}\hline\arrayrulecolor{black}
        \multirow{2}{*}{Completeness} & CN & All references in included publications successfully linked \\
        \ & SDR & No sections or content missing (appendices, formulae, etc.) \\
        \bottomrule
      \end{tabular}
      \begin{tablenotes}
...
\begin{enumerate}
    ...
    \item \textbf{Comparability} as a quality criterion is of importance to enable the combined use of data sets. In other words, items in the data set should be clearly identified. In the case of scholarly data, this calls for determining the persistent identifiers (e.g. DOI, ORCiD, and ROR~\cite{Meadows2019}) of items of interest. Following from comparability is therefore the quality requirement $\rightarrow$\,\textit{use of persistent identifiers}.
    \item \textbf{Completeness} of data means that no records are missing \emph{and} records are not missing any attributes. Regarding scholarly data, there are practical boundaries to simply aiming for including everything ever published (e.g. closed access publications and distribution rights). On the level of a publication record, however, it makes sense to strive for completeness of metadata (e.g. not missing author information) and complete coverage of references (i.e. not missing any references in the citation network). Based on this, we can formulate the quality criterion $\rightarrow$\,\textit{completeness of metadata and reference data}.
\end{enumerate}

In short, quality criterion especially relevant for scholarly data are as follows.
