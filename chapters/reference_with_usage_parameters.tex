...
\begin{figure}
    ...
    \caption[Observations of initial annotation round]{Observations of initial annotation round.}
...
Lastly, in Figure~\ref{fig:rel-dists}, show distances between artifacts and their parameters, as well as parameters and their values. We see that artifacts usually are mentioned before their parameters (78\%), and parameters before their values (93\%). The reverse cases also exist, but are less common.
...
\begin{figure}[bt]
  ...
  \caption[Relation extraction with emphasis on entity candidate pair types and distance]{Relation extraction with emphasis on entity candidate pair types and distance.}
...
\begin{figure}[tb]
  ...
  \caption[Fine-tuned model evaluation (5-fold cross validation)]{Fine-tuned model evaluation (5-fold cross validation).}
...
\begin{figure}
  ...
  \caption[Frequency of hyperparameter mention positions in papers]{Frequency of hyperparameter mention positions in papers.}
...
As a baseline, we use a JSON variant for each model, where the \texttt{[format]} and \texttt{[examples]} components of prompts use JSON, and compare it to the respective YAML version. All models are used with greedy decoding (temperature = 0) for the sake of reproducibility.
...
Lastly, we see that using YAML leads to better ER results across all six models, with ER performance being comparable or improved as well.
